...
return function(secondsToTime, totalTime, songs, thumbnails, arialBigBold, arialMedBold, arialSmaBold, triangleImg,
    ...
    local function play()
        ...
        love.graphics.setBackgroundColor(song.colour)
        currentPlaying = song
    end
    ...
    local function drawTextWithOutline(text, font, x, y)
        ...
        if not textOutlineCached[text .. tostring(font)] then
            ...
            print("cached " .. tostring(font) .. " " .. text)
        else
            local canvas = textOutlineCached[text .. tostring(font)]
            love.graphics.draw(canvas, math.floor(x), math.floor(y))
        end
    ...
    function renderer.draw()
        ...
        if not song then
            return drawTextWithOutlineRealtimef("No songs loaded", arialBigBold, 0, 0, WindowWidth, "center")
        end
        ...
        if song then
            ...
            drawTextWithOutlineRealtime(song.actualName, arialBigBold, 20, 10)
            drawTextWithOutlineRealtime(
                string.format("%s / %s", secondsToTime(song.audio:tell()),
                    secondsToTime(song.length)),
                arialMedBold, 20, arialBigBold:getHeight() + 10 + 10
            )
            local songText = "Song #" .. currentIndex
            ...
            if prev.actualName ~= songs[#songs].actualName then
                love.graphics.draw(triangleImg, 40,
                    WindowHeight - (triangleImg:getHeight() * 0.25) - 110 + yoff,
                    math.pi / 2, 0.25,
                    0.2)

                drawTextWithOutlineRealtime(cutString(prev.actualName, 40), arialMedBold, 60,
                    ty)
            end

            if upNext.actualName ~= song.actualName then
                drawTextWithOutlineRealtimef(cutString(upNext.actualName, 40), arialMedBold, -60,
                    ty,
                    WindowWidth, "right")
        ...
        if time < 4 then
            love.graphics.setColor(0, 0, 0, 1 - (time / 4))
    ...
    if not isRendering then
        ...
        function love.keypressed(key)
            ...
            elseif key == "left" then
                seek(-10)
            elseif key == "space" and not searching then
                isPaused = not isPaused
            elseif key == "/" and not searching then
                searching = true
                searchText = ""
                songSelectIndex = math.max(1, math.floor((love.mouse.getY() - 9) / (arialSmaBold:getHeight() + 2)))
                love.keyboard.setTextInput(searching)
                curSongSearch = searchSongs(searchText)
            elseif key == "escape" and searching then
                searching = false
                searchText = ""
                songSelectScroll = 0
                love.keyboard.setTextInput(searching)
                curSongSearch = searchSongs(searchText)
            elseif key == "backspace" and searching then
                searchText = string.sub(searchText, 1, #searchText - 1)
                curSongSearch = searchSongs(searchText)
            elseif key == "down" and searching then
                songSelectIndex = songSelectIndex + 1
                if songSelectIndex > #curSongSearch then
                    songSelectIndex = 1
                end
            elseif key == "up" and searching then
                songSelectIndex = songSelectIndex - 1
                if songSelectIndex < 1 then
                    songSelectIndex = #curSongSearch
                end
            elseif key == "return" and searching then
                playSongFromSearch()
            end
        end

        function love.textinput(t)
            if playedSong and t == "/" then
                playedSong = false
                return
            end
            if searching then
